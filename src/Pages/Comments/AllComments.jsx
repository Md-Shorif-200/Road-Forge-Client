      ...
      {!Comments || Comments.length == 0 ? (
        <div className="w-full min-h-screen flex justify-center items-center flex-col">
          <img
            ...
            alt="comments not found"
            className="w-1/2"
          />
          <div className="text-xl capitalize font-semibold text-gray-800 mt-6">
            <p>no comments here</p>
            <Link
              className="text-black font-semibold mt-4  flex items-center gap-x-4  "
              to="/"
            >
              Go Home <CiLogout className="text-3xl"></CiLogout>{" "}
            </Link>
          </div>
        </div>
      ) : (
            ...
            <div className="all_comments_card">
              {Comments.map((data, index) => {
                return (
                  <div
                    key={index}
                    className="w-full px-[2%] sm:px-[4%] pt-3 sm:pt-4 md:pt-6  sm:pb-3 border border-gray-200"
                  >
                    {/* comments section */}
                    ...
                    <div className="comment_card flex  ">
                      ...
                      <div className="user_comment w-full  pl-2  sm:pl-4 py-0">
                        <div className="road_map_item my-2">
                          <h3 className="text-base sm:text-lg capitalize font-semibold">
                            {" "}
                            {data?.roadmapItemTitle}{" "}
                        ...
                        <div className="comment_reply_btn  mr-2 flex justify-between mt-1  ">
                          <div className="flex gap-x-6 ">
                            ...
                            <a
                              className="capitalize font-semibold text-sm sm:text-base cursor-pointer hover:underline"
                              onClick={() => handleReplayBtn(data?._id)}
                            >
                              {" "}
                              reply
                            </a>
                          ...
                          <div className="show_replies">
                            <button
                              className=" text-sm sm:text-base font-semibold capitalize hover:underline"
                              disabled={!data.replies}
                              onClick={() => handleReplyShowButton(data?._id)}
                            >
                              {" "}
