...
const ReplyMessage = ({commentData}) => {
            ...
            // const nestedReplies = commentData.replies.nestedReplies; // get nested-replies data
            console.log(replies);
            
            const [nestedReplyId,setNestedReplyId] = useState(null) ;
            const [showNestedReplyCard,setShowNestedReplyCard] = useState(null)
       
            // const {name,email,photo,replyMessage,time} = replies;
                                        ...
                                        <div className='reply_message_card flex  gap-x-3 mb-5'>
                                            ...
                                            <div className="reply_message w-full ">
                                                   <div className='message bg-[#F0F2F5] p-2 rounded-xl' >
                                                    <h2 className="name text-[14px] capitalize font-semibold"> {data?.name} </h2>
                                                   <p className='text-base capitalize text-gray-700'>  {data?.replyMessage} </p>
                                                   </div>

                                                           {/*  nested reply button  */}
                <div className="  mr-2 flex justify-between mt-1  ">
                          <div className="flex gap-x-6  ">
                               <p className=" font-semibold text-sm lowercase">1 day ago</p>
                  <a  className="capitalize text-sm font-semibold  cursor-pointer hover:underline" onClick={() => handleNestedReplayBtn(data?.id)}> reply</a>
                          </div>
                          {/* show nested-reply button */}
                          <div className="show_replies">
                                <a className="font-semibold capitalize hover:underline text-sm cursor-pointer" onClick={() => showNestedReplies(data?.id)} > replies ({data.nestedReplies?.length || 0})  </a>
                          </div>
                          </div>
                                            </div>
                                        </div> 
                                         ...
                                         <div>
                                             {
                                                showNestedReplyCard === data?.id ? 
                                                 ...
                                                 <div className=" ">
                                                             {
                                                                 data.nestedReplies.map((nestedReplyData,index) => <NestedReplyCard key={index} nestedReplyData={nestedReplyData}></NestedReplyCard> )
                                                             }
                                                   </div>
                                                 </>
