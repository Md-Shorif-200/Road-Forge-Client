          ...
          <div className="status_badge   w-[170px] text-center">
            <p
              className={`text-sm font-semibold py-1.5 text-white rounded-lg ${
                myRoadMapData?.status == "Planned"
                  ? "bg-orange-400"
                  : myRoadMapData?.status == "In Progress"
                  ? "bg-blue-400"
                  : "bg-green-400"
              }`}
            >
              {" "}
              {myRoadMapData?.status}{" "}
            </p>
          </div>
          <div className="upvote_count">
            <p className="text-gray-600 font-bold text-lg">
              {" "}
              Upvotes : {myRoadMapData?.upvotes}{" "}
            </p>
          </div>
        </div>
        <h1 className="text-2xl  px-4 sm:px-7 md:px-10 font-semibold  capitalize my-4 sm:my-3">
          {" "}
          {myRoadMapData?.title}{" "}
        </h1>
        <p className="text-base  px-4 sm:px-7 md:px-10 lowercase text-gray-600 py-3">
          {" "}
          {myRoadMapData?.description}{" "}
        </p>
